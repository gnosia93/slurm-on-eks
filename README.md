...
본 워크숍은 Amazon EKS에서 Slurm과 Slinky를 결합하여 Llama 3와 같은 대규모 언어 모델(LLM)을 최적으로 학습시키는 실전 가이드를 제공한다.
전통적인 HPC 스케줄러인 Slurm은 대규모 분산 학습의 필수 조건인 '갱 스케줄링(Gang Scheduling)'을 통해 GPU 자원 활용의 안정성을 극대화한다. 여기에 오픈소스 프로젝트인 Slinky를 더해, Slurm의 정교한 스케줄링 능력을 쿠버네티스 파드(Pod)에 투명하게 이식한다.
참가자들은 이 워크숍을 통해 복잡한 인프라 관리 부담 없이, 쿠버네티스의 유연함과 Slurm의 강력한 자원 관리 기능을 동시에 확보하는 아키텍처를 직접 경험하게 될 것이다.
