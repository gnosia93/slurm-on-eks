...
[Slurm 아키텍처의 이해]를 먼저 읽어 보시기 바랍니다. Slurm은 태생이 HPC(고성능 컴퓨팅)용이라, 쿠버네티스와 달리 별도의 설정을 하지 않아도 기본적으로 "자원이 100% 확보되지 않으면 아예 시작도 안 하는" 갱 스케줄링 방식으로 작동한다. K8s는 컨테이너 관리와 오케스트레이션을 담당하고,Slurm은 실제 GPU 자원의 할당과 작업 순서(Scheduling)를 담당하게 하며,
Slinky가 이 둘 사이에서 "K8s에서 요청한 작업을 Slurm이 이해하게 전달하고, Slurm이 할당한 자원을 K8s 포드(Pod)에 맵핑"해주는 가교가 됩니다.
