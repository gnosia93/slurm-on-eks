...
* 표준화된 에이전트 인터페이스: 하이브리드 환경에서 서로 다른 인프라(온프레미스 베어메탈, 클라우드 가상 머신 등)를 동일한 방식으로 제어할 수 있도록 통신 프로토콜을 표준화한다. 이를 통해 관리자는 하드웨어의 물리적 위치에 상관없이 단일 Slurm 컨트롤러 하에서 통합된 자원 풀을 운영할 수 있다.
